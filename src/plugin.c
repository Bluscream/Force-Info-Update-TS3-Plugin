...
#define PLUGIN_NAME "Force Info Update"
...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"Force Info Update";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Force Info Update";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
	return "Force Info Update";
#endif
}
...
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
	return "Forces the info frames to update.";
}
...
void ts3plugin_infoData(uint64 serverConnectionHandlerID, uint64 id, enum PluginItemType type, char** data) {
	...
	case PLUGIN_CHANNEL:
		...
		return;
	case PLUGIN_CLIENT:
		//ts3Functions.requestClientVariables(serverConnectionHandlerID, (anyID)id, clientData);
		data = NULL;
		return;
	default:
		data = NULL;
	...
	//ts3Functions.freeMemory(clientData);
}
